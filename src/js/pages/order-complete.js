...
root.innerHTML = order ? `<section class="order-complete section">
  ...
  <dl class="receipt"><div><dt>測試訂單編號</dt><dd>${order.id}</dd></div><div><dt>付款方式</dt><dd>${methodName[order.payment.method]}</dd></div><div><dt>測試參考碼</dt><dd>${order.payment.reference}</dd></div><div><dt>測試金額</dt><dd>${money(order.totals.total)}</dd></div><div><dt>狀態</dt><dd>${order.status}</dd></div></dl>
